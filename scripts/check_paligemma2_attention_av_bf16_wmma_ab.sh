#!/usr/bin/env bash
# Runs deterministic generation A/B checks for the HIP PaliGemma2 attention
# AV BF16 WMMA experiment. The baseline keeps attention in F32; the candidate
# keeps QK in F32 but stores softmax scores/V as BF16 for a local WMMA AV.
...
run_generation() {
  ...
  local cache_args=("$@")
  local av_args=()
  if [[ "${attention_mode}" == "av_bf16_wmma" ]]; then
    av_args=(--paligemma_vit_hip_attention_av_bf16_wmma 1)
  fi
