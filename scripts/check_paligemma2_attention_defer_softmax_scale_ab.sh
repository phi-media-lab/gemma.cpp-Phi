#!/usr/bin/env bash
# Runs deterministic generation A/B checks for the HIP PaliGemma2 deferred
# softmax-scale attention variant. The baseline is the current F32 attention
# pack-to-BF16 path; the candidate keeps QK/AV in F32 but stores exp() scores
# and applies the saved reciprocal softmax denominator while packing to BF16.
...
run_generation() {
  ...
  shift 4
  local cache_args=("$@")
  local attention_args=(--paligemma_vit_hip_attention_pack_bf16 1)
  if [[ "${attention_mode}" == "defer_scale" ]]; then
    attention_args+=(--paligemma_vit_hip_attention_defer_softmax_scale 1)
  fi
...
check_model() {
  ...
  for image in "${IMAGES[@]}"; do
    for prompt in "${PROMPTS[@]}"; do
      ...
      baseline_output="$(run_generation "${image}" "${weights}" "${prompt}" \
        pack_bf16 "${cache_args[@]}")"
      candidate_output="$(run_generation "${image}" "${weights}" "${prompt}" \
        defer_scale "${cache_args[@]}")"
      if [[ "${baseline_output}" != "${candidate_output}" ]]; then
        echo "FAIL ${label}: image=${image} prompt=${prompt}" >&2
        echo "  pack_bf16   : ${baseline_output}" >&2
        echo "  defer_scale : ${candidate_output}" >&2
        failures=$((failures + 1))
