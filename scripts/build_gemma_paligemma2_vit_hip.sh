#!/usr/bin/env bash
# Builds an experimental gemma CLI with the PaliGemma2 HIP image-backend hook.
...
OUT="${OUT:-${ROOT_DIR}/build/gemma_paligemma2_vit_hip}"
...
"${HIPCC}" \
  ...
  -O3 \
  -std=c++17 \
  -DGCPP_EXPERIMENTAL_PALIGEMMA2_VIT_HIP \
  -c \
  -I"${ROOT_DIR}" \
  -I"${ROOT_DIR}/build/_deps/highway-src" \
  -I"${ROOT_DIR}/build/_deps/sentencepiece-src/src" \
